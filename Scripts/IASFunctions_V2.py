#################################################################################
# FUNCTIONS OF INTEFEROGRAM ANALYSIS V.1.X.X
################################################################################
...
def fringes_width(data, fang_deg):
    ...
    result = np.ones(np.shape(data))*np.mean(f_width)
...
def normalize_to_256(input_path):
    # Load 16-bit image
    ...
    # Normalize to [0, 255]
    arr_norm = (arr - low) * 255.0 / (high - low)
    arr_8bit = arr_norm.astype(np.uint8)
